...
# Build aplikasi golang menjadi binary dengan nama go-learn-docker
RUN CGO_ENABLED=0 GOOS=linux go build -o ./cmd/go-learn-docker ./cmd/main.go

# Statement EXPOSE digunakan untuk expose port container
EXPOSE 9999

# Run
CMD ["./cmd/go-learn-docker"]